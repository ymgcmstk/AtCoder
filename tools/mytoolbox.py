...
try:
    ...
    get_test_file = ymgc_toolbox.get_test_file
except:
    ...
    def cp2www(*input1, **input2):
        ...
        return
    def get_test_file(*input1, **input2):
...
try:
    ...
    def comp_vecs(vec_a, vec_b, this_eps1=1e-7, this_eps2=2e-2):
        assert vec_a.ravel().shape == vec_b.ravel().shape
        try:
            assert np.linalg.norm(vec_a.ravel() - vec_b.ravel()) < this_eps1
            # assert spatial.distance.cosine(vec_a.reshape((1, vec_a.size)), vec_b.reshape((1, vec_b.size))) < this_eps2
            assert spatial.distance.cosine(vec_a.ravel(), vec_b.ravel()) < this_eps2
        except:
            print 'assertion failed.'
            print 'diff norm : %f' % (np.linalg.norm(vec_a.ravel() - vec_b.ravel()))
            print 'cos similarity : %f' % (spatial.distance.cosine(vec_a.ravel(), vec_b.ravel()))
except:
...
class DeprecatedFunc:
    def __init__(self, old_func_name, func2, print_str=None):
        self.genuine_func = func2
        self.old_func_name = old_func_name
        if print_str is None:
            self.print_str = 'This function "%s" method is deprecated. Please use the function "%s"'
        else:
            self.print_str = print_str
    def __call__(self, *inputs1, **inputs2):
        printr(self.print_str % (self.old_func_name, self.genuine_func.__name__))
        self.genuine_func(*inputs1, **inputs2)

wait_for_appearing = DeprecatedFunc('wait_for_appearing', wait_for)
check_two_vecs     = DeprecatedFunc('check_two_vecs', comp_vecs)

"""
# This does not work.
def deprecate(dep_func_name, new_func):
    exec("global %s" % dep_func_name)
