...
//      tr.append($('<td/>').addClass('standings-rank').html('<a href="/standings#page_' + ATCODER.standings.me.page + '">' + row.rank + '</a>'));
  ...
  if (row.modified != null) {
    ...
    tr.append($('<td/>').addClass('standings-contest-name')
      .append(
    ...
    // username
    var td_username =
      $('<td/>').addClass('standings-username')
      .append(
        $('<a/>').attr('href', '/users/' + encodeURIComponent(row.user_screen_name)).text(row.user_name)
      );
    ...
    if(row.contest_screen_name != null) {
      td_username.append(
        ...
        .html(' <i class="icon-search tooltip-label" rel="tooltip" data-title="' + escapeAttrValue(escapeAttrValue(row.user_name)) + 'の提出を確認" data-original-title=""></i>')
      );
    } else {
      td_username.append(
