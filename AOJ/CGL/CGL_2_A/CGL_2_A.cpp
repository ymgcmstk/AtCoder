...
public:
  ...
  bool isParallel(Point &p) {return fabs(this->cross(p)) < this->EPS;}
  bool isOrthogonal(Point &p) {return fabs(this->dot(p)) < this->EPS;}
};
...
int main() {
  ...
  rep(i, N) {
    cin >> tx >> ty;
    Point<double> a(tx, ty);
    cin >> tx >> ty;
    a.x -= tx;
    a.y -= ty;

    cin >> tx >> ty;
    Point<double> b(tx, ty);
    cin >> tx >> ty;
    b.x -= tx;
    b.y -= ty;
